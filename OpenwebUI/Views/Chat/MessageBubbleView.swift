...
struct MessageBubbleView: View {
    ...
    private var userMessage: some View {
        VStack(alignment: .trailing, spacing: 8) {
            ...
            if !isEditing && !isStreaming {
                HStack(spacing: 4) {
                    // Edit button
                    ActionButton(icon: "pencil", help: String(localized: "message.edit.help")) {
        ...
        .contextMenu {
                    Button(String(localized: "message.copy.menu")) {
                        copyToClipboard(message.content)
                    }
                    if onEdit != nil {
                        Button(String(localized: "message.edit.menu")) {
                            editText = message.content
                            isEditing = true
                        }
                    }
    ...
    private var editView: some View {
        VStack(alignment: .trailing, spacing: 8) {
            ...
            HStack(spacing: 8) {
                    Button(String(localized: "message.cancel")) {
                    isEditing = false
                    editText = ""
                }
                .buttonStyle(.plain)
                ...
                Button(String(localized: "message.save")) {
    ...
    private var assistantMessage: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                if !isStreaming && !message.content.isEmpty {
                    HStack(spacing: 4) {
                        ...
                        // Play / Stop TTS
                        if isSpeakingThis {
                            ActionButton(icon: "stop.fill", help: String(localized: "message.stopSpeaking.help")) {
                                onStopSpeaking?()
                            }
                        } else {
                            ActionButton(icon: "speaker.wave.2", help: String(localized: "message.readAloud.help")) {
                        ...
                        if isLastAssistant {
                            ActionButton(icon: "arrow.counterclockwise", help: String(localized: "message.regenerate.help")) {
            ...
            .contextMenu {
                    Button(String(localized: "message.copyMessage.menu")) {
                        copyToClipboard(message.content)
                    }
                    if onSpeak != nil {
                        Button(isSpeakingThis ? String(localized: "message.stopSpeaking.menu") : String(localized: "message.readAloud.menu")) {
                            if isSpeakingThis {
                                onStopSpeaking?()
                            } else {
                                onSpeak?(message.content, message.id)
                    ...
                    if isLastAssistant, onRegenerate != nil {
                        Button(String(localized: "message.regenerate.menu")) {
                            onRegenerate?(message.id)
                        }
                    }
            }
...
private struct ReasoningBlockView: View {
    ...
    private var headerText: String {
        if !block.isDone {
            return String(localized: "reasoning.thinking")
        }
        if let duration = block.duration, let seconds = Double(duration) {
            if seconds < 1 {
                return String(localized: "reasoning.thoughtLessThanSecond")
            } else if seconds < 60 {
                let s = Int(seconds)
                return String(format: String(localized: s == 1 ? "reasoning.thoughtSeconds" : "reasoning.thoughtSeconds.plural"), s)
            } else {
                let minutes = Int(seconds) / 60
                return String(format: String(localized: minutes == 1 ? "reasoning.thoughtMinutes" : "reasoning.thoughtMinutes.plural"), minutes)
            }
        }
        return String(localized: "reasoning.thought")
    }
}
...
private struct ToolCallView: View {
    ...
    private var headerLabel: some View {
        ...
        case .pending:
            Text(toolCall.function.name)
                ...
                .foregroundStyle(AppColors.textSecondary)
        case .executing:
            (Text(String(localized: "toolCall.executing")).foregroundStyle(AppColors.textTertiary)
        ...
        case .completed:
            if toolCall.result != nil {
                (Text(String(localized: "toolCall.viewResult")).foregroundStyle(AppColors.textTertiary)
                 + Text(toolCall.function.name).foregroundStyle(AppColors.textSecondary).bold())
                    .font(AppFont.caption(size: 12))
            } else {
                (Text(String(localized: "toolCall.executed")).foregroundStyle(AppColors.textTertiary)
                 + Text(toolCall.function.name).foregroundStyle(AppColors.textSecondary).bold())
                    .font(AppFont.caption(size: 12))
            }
        case .error:
            (Text(String(localized: "toolCall.failed")).foregroundStyle(AppColors.red500)
             + Text(toolCall.function.name).foregroundStyle(AppColors.textSecondary).bold())
                .font(AppFont.caption(size: 12))
        }
    }
    ...
    private var inputSection: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(String(localized: "toolCall.input"))
    ...
    private func outputSection(_ result: String) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(String(localized: "toolCall.output"))
                .font(.system(size: 10, weight: .medium))
                .tracking(1)
                .foregroundStyle(AppColors.textTertiary)
...
private struct CopyButton: View {
    ...
    var body: some View {
        ...
        .buttonStyle(.plain)
        .help(String(localized: "message.copyMessage.help"))
    }
}
            ...
            Button {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(sources.count == 1
                        ? String(format: String(localized: "sources.singular"), sources.count)
                        : String(format: String(localized: "sources.plural"), sources.count))
                        .font(.system(size: 11, weight: .medium))
                    Image(systemName: isExpanded ? "chevron.up" : "chevron.down")
                        .font(.system(size: 9))
                }
        ...
        if let urlString = source.url, let url = URL(string: urlString) {
            ...
            HStack(alignment: .top, spacing: 6) {
                Image(systemName: "doc.text")
                    ...
                    .padding(.top, 2)
                VStack(alignment: .leading, spacing: 1) {
                    Text(source.title ?? String(localized: "sources.fallback"))
                        .font(.system(size: 11, weight: .medium))
                        .foregroundStyle(AppColors.textPrimary)
                        .lineLimit(1)
                    if let snippet = source.snippet {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(execution.name ?? String(localized: "codeExecution.default"))
                        .font(.system(size: 11, weight: .medium))
                        .foregroundStyle(AppColors.textSecondary)
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    if let code = execution.code, !code.isEmpty {
                        VStack(alignment: .leading, spacing: 2) {
                            Text(String(localized: "codeExecution.code"))
                                .font(.system(size: 9, weight: .semibold))
                                .foregroundStyle(AppColors.textTertiary)
                    ...
                    if let result = execution.result {
                        if let output = result.output, !output.isEmpty {
                            VStack(alignment: .leading, spacing: 2) {
                            Text(String(localized: "codeExecution.output"))
                                .font(.system(size: 9, weight: .semibold))
                                .foregroundStyle(AppColors.textTertiary)
                                ScrollView(.horizontal, showsIndicators: false) {
                                    Text(output)
                        ...
                        if let error = result.error, !error.isEmpty {
                            VStack(alignment: .leading, spacing: 2) {
                            Text(String(localized: "codeExecution.error"))
                                .font(.system(size: 9, weight: .semibold))
                                .foregroundStyle(AppColors.red500)
                                Text(error)
                                    .font(.system(size: 11, design: .monospaced))
