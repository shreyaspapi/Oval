...
struct ChatSidebarView: View {
    ...
    var body: some View {
                            ...
                            ForEach(group.conversations) { conversation in
                                ...
                                .contextMenu {
                                    Button {
                                        ...
                                        Label(String(localized: "sidebar.menu.share"), systemImage: "square.and.arrow.up")
                                    }

                                    Button {
                                        Task { await appState.downloadConversation(conversation.id) }
                                    } label: {
                                        Label(String(localized: "sidebar.menu.download"), systemImage: "arrow.down.circle")
                                    }
                                    ...
                                    Button {
                                        ...
                                        Label(String(localized: "sidebar.menu.rename"), systemImage: "pencil")
                                    }
                                    ...
                                    Button {
                                        ...
                                        Label(conversation.isPinned ? String(localized: "sidebar.menu.unpin") : String(localized: "sidebar.menu.pin"), systemImage: conversation.isPinned ? "bookmark.slash" : "bookmark")
                                    }

                                    Button {
                                        Task { await appState.cloneConversation(conversation.id) }
                                    } label: {
                                        Label(String(localized: "sidebar.menu.clone"), systemImage: "doc.on.doc")
                                    }
                                    ...
                                    if !folders.isEmpty {
                                        Menu {
                                            ...
                                            if conversation.folder_id != nil {
                                                Divider()
                                                Button(String(localized: "sidebar.menu.removeFromFolder")) {
                                                    Task { await appState.moveConversation(conversation.id, toFolder: nil) }
                                                }
                                            }
                                        } label: {
                                            Label(String(localized: "sidebar.menu.move"), systemImage: "folder")
                                        }
                                    }
                                    ...
                                    Button {
                                        ...
                                        Label(String(localized: "sidebar.menu.archive"), systemImage: "archivebox")
                                    }
                                    ...
                                    Button(role: .destructive) {
                                        ...
                                        Label(String(localized: "sidebar.menu.delete"), systemImage: "trash")
                                    }
                                }
                            }
                        } header: {
            ...
            VStack(spacing: 0) {
                ...
                HStack(spacing: 10) {
                    ...
                    if let user = appState.currentUser {
                        Text(user.name ?? String(localized: "sidebar.userFallback"))
                            .font(AppFont.body(size: 13))
                    ...
                    .buttonStyle(.plain)
                    .help(String(localized: "sidebar.settings.help"))
                }
        ...
        .confirmationDialog(
            String(localized: "sidebar.deleteChat.title"),
            isPresented: Binding(
            ...
            Button(String(localized: "sidebar.deleteChat.confirm"), role: .destructive) {
                if let id = deleteTarget {
                    deleteTarget = nil
                    Task { await appState.deleteConversation(id) }
                }
            }
            Button(String(localized: "sidebar.deleteChat.cancel"), role: .cancel) {
                deleteTarget = nil
            }
        } message: {
            Text(String(localized: "sidebar.deleteChat.message"))
        }
        .alert(String(localized: "sidebar.rename.title"), isPresented: Binding(
            get: { renameTarget != nil },
            set: { if !$0 { renameTarget = nil } }
        )) {
            TextField(String(localized: "sidebar.rename.titlePlaceholder"), text: $renameText)
            Button(String(localized: "sidebar.rename.confirm")) {
            ...
            Button(String(localized: "sidebar.rename.cancel"), role: .cancel) {
                renameTarget = nil
            }
        } message: {
            Text(String(localized: "sidebar.rename.message"))
        }
    }
    ...
    private var groupedConversations: [ConversationGroup] {
        ...
        guard let startOfYesterday = calendar.date(byAdding: .day, value: -1, to: startOfToday),
            ...
            return [ConversationGroup(label: String(localized: "conversations.label"), conversations: appState.filteredConversations)]
        }
        ...
        if !today.isEmpty { groups.append(ConversationGroup(label: String(localized: "time.today"), conversations: today)) }
        if !yesterday.isEmpty { groups.append(ConversationGroup(label: String(localized: "time.yesterday"), conversations: yesterday)) }
        if !prev7.isEmpty { groups.append(ConversationGroup(label: String(localized: "time.previous7Days"), conversations: prev7)) }
        if !prev30.isEmpty { groups.append(ConversationGroup(label: String(localized: "time.previous30Days"), conversations: prev30)) }
...
struct PinnedModelsSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
            // Section header
            Text(String(localized: "sidebar.pinnedModels"))
            ...
            ForEach(appState.pinnedModels) { model in
                ...
                .contextMenu {
                    Button {
                        ...
                        Label(String(localized: "pinnedModel.unpin"), systemImage: "pin.slash")
                    }

                    Button {
                        appState.selectedModel = model
                    } label: {
                        Label(String(localized: "pinnedModel.select"), systemImage: "checkmark.circle")
                    }
                    ...
                    if !appState.isDefaultModel(model) {
                        Button {
                            ...
                            Label(String(localized: "pinnedModel.setDefault"), systemImage: "star")
                        }
                    }
                }
            }
