...
struct ServerRailView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                .buttonStyle(.plain)
                .help(String(localized: "serverRail.addServer.help"))
                .onHover { hovering in
                    if hovering {
    ...
    private func serverIcon(for server: ServerConfig) -> some View {
        ...
        HStack(spacing: 0) {
            ...
            .contextMenu {
                Button(String(localized: "serverRail.copyServerURL")) {
                    NSPasteboard.general.clearContents()
                    NSPasteboard.general.setString(server.url, forType: .string)
                }
                Divider()
                Button(String(localized: "serverRail.removeServer"), role: .destructive) {
