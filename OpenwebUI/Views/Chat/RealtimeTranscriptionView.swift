...
struct RealtimeTranscriptionView: View {
        ...
        HStack(spacing: 8) {
            ...
            if manager.isActive {
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "transcription.live"))
                        .font(AppFont.caption(size: 10))
                        .foregroundStyle(.red)
                        .fontWeight(.bold)
                }
            } else if !manager.isModelReady {
                    Text(String(localized: "transcription.modelNotLoaded"))
                    .font(AppFont.caption(size: 11))
    ...
    private var footerBar: some View {
        HStack(spacing: 12) {
            ...
            .help(diarizationManager.isEnabled ? String(localized: "transcription.speaker.disable.help") : String(localized: "transcription.speaker.enable.help"))

            Spacer()

            // Speaker count badge
            if diarizationManager.isEnabled && diarizationManager.speakerCount > 0 {
                Text(diarizationManager.speakerCount == 1
                    ? String(format: String(localized: "transcription.speakerCount.singular"), diarizationManager.speakerCount)
                    : String(format: String(localized: "transcription.speakerCount.plural"), diarizationManager.speakerCount))
                    .font(AppFont.caption(size: 10))
