...
struct ConnectView: View {
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 12) {
                    ...
                    Text(String(localized: "connect.subtitle"))
                        .font(.system(size: 15))
                        .foregroundStyle(.white.opacity(0.5))
                }
                ...
                VStack(spacing: 16) {
                    ...
                    // Auth method toggle
                    HStack(spacing: 0) {
                        authTab(label: String(localized: "connect.emailTab"), icon: "envelope", method: .emailPassword)
                        authTab(label: String(localized: "connect.apiKeyTab"), icon: "key", method: .apiKey)
                        authTab(label: String(localized: "connect.ssoTab"), icon: "globe", method: .sso)
                    }
    ...
    private var emailPasswordFields: some View {
        VStack(spacing: 12) {
            formField(
                ...
                icon: "envelope",
                content: AnyView(
                    TextField("", text: $appState.emailInput, prompt: Text(String(localized: "connect.emailPrompt")).foregroundStyle(.white.opacity(0.25)))
                        .textFieldStyle(.plain)
                        .font(.system(size: 14))
                        .foregroundStyle(.white)
            ...
            formField(
                label: String(localized: "connect.password"),
                icon: "lock",
                content: AnyView(
                    SecureField("", text: $appState.passwordInput, prompt: Text(String(localized: "connect.passwordPrompt")).foregroundStyle(.white.opacity(0.25)))
                        .textFieldStyle(.plain)
                        .font(.system(size: 14))
                        .foregroundStyle(.white)
                        .onSubmit {
    ...
    private var ssoFields: some View {
        VStack(spacing: 12) {
            Text(String(localized: "connect.ssoDescription"))
                .font(.system(size: 12))
            ...
            Button {
                ...
                guard !url.isEmpty else {
                    appState.connectionError = String(localized: "connect.errorEnterURL")
                    return
                }
                showSSOWebView = true
            } label: {
                HStack(spacing: 6) {
                    Image(systemName: "arrow.up.right.square")
                        .font(.system(size: 13))
                    Text(String(localized: "connect.openSSOLogin"))
                        .font(.system(size: 13, weight: .semibold))
                }
                .foregroundStyle(.white)
                .frame(maxWidth: .infinity)
    ...
    private var apiKeyFields: some View {
        VStack(spacing: 8) {
            ...
            Text(String(localized: "connect.apiKeyHint"))
                .font(.system(size: 11))
                .foregroundStyle(.white.opacity(0.25))
                .multilineTextAlignment(.center)
        }
