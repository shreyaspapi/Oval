...
struct ChatMessage: Identifiable, Equatable {
    ...
    /// Source/citation references (knowledge retrieval, web search results)
    var sources: [ChatSourceReference]?
    /// Code execution results from sandbox/interpreter tools
    var codeExecutions: [ChatCodeExecution]?
    /// Follow-up suggestions generated by the server
    var followUps: [String]?
    /// Token usage statistics
    var usage: TokenUsage?
    /// Structured error from the server (e.g. tool execution failure)
    ...
    static func == (lhs: ChatMessage, rhs: ChatMessage) -> Bool {
        ...
        lhs.parentId == rhs.parentId && lhs.toolCalls == rhs.toolCalls &&
        lhs.toolCallId == rhs.toolCallId && lhs.sources == rhs.sources &&
...
extension ChatMessage: Codable {
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try c.encodeIfPresent(codeExecutions, forKey: .codeExecutions)
...
struct ChatFeatures: Codable {
    ...
    init(web_search: Bool? = nil, image_generation: Bool? = nil, memory: Bool? = nil, code_interpreter: Bool? = nil) {
...
/// Options for streaming (e.g. include usage stats).
...
struct ChatCompletionRequest: Codable {
    ...
    let chat_id: String?
    /// Assistant message ID — server uses this to route Socket.IO events to the correct message
    let id: String?
    /// Parent message ID (the user message this response is for)
    let parent_id: String?
    /// Parent message object — must be included (even as empty {}) to prevent NoneType on OWUI 0.6.42+
    let parent_message: [String: String]?
    /// Streaming options (e.g. include_usage for token counts)
...
// MARK: - Source / Citation References

/// A source or citation reference from knowledge retrieval or web search.
struct ChatSourceReference: Codable, Identifiable, Equatable {
    var id: String
    var title: String?
    var url: String?
    var snippet: String?
    var type: String?         // "web_search", "knowledge", "code_execution", etc.
    var metadata: [String: String]?

    init(id: String = UUID().uuidString, title: String? = nil, url: String? = nil, snippet: String? = nil, type: String? = nil, metadata: [String: String]? = nil) {
        self.id = id
        self.title = title
        self.url = url
    ...
    /// Parse from a raw Socket.IO source/citation event payload.
    static func fromSocketPayload(_ data: [String: Any]) -> ChatSourceReference? {
        // The server sends sources in OpenWebUI format: { source: { name, url, ... }, document: [...], metadata: [...] }
        var ref = ChatSourceReference()
        if let source = data["source"] as? [String: Any] {
            ref.id = source["id"] as? String ?? UUID().uuidString
            ref.title = source["name"] as? String
            ref.url = source["url"] as? String
            ref.type = source["type"] as? String
        } else {
            ref.id = data["id"] as? String ?? UUID().uuidString
            ref.title = data["name"] as? String ?? data["title"] as? String
            ref.url = data["url"] as? String
            ref.type = data["type"] as? String
        }
        // Extract snippet from document array
        if let documents = data["document"] as? [String], let first = documents.first {
            ref.snippet = first
        }
        return ref
    }
...
/// A code execution result from a sandbox/interpreter tool.
struct ChatCodeExecution: Codable, Identifiable, Equatable {
    let id: String
    var name: String?
    var language: String?
    var code: String?
    var result: ChatCodeExecutionResult?

    init(id: String = UUID().uuidString, name: String? = nil, language: String? = nil, code: String? = nil, result: ChatCodeExecutionResult? = nil) {
        self.id = id
        self.name = name
    ...
    /// Parse from a raw Socket.IO code_execution event payload.
    static func fromSocketPayload(_ data: [String: Any]) -> ChatCodeExecution? {
        let id = data["id"] as? String ?? UUID().uuidString
        return ChatCodeExecution(
            id: id,
            name: data["name"] as? String,
            language: data["language"] as? String,
            code: data["code"] as? String,
            result: {
                guard let r = data["result"] as? [String: Any] else { return nil }
                return ChatCodeExecutionResult(
...
struct ChatCodeExecutionResult: Codable, Equatable {
    let output: String?
    let error: String?
}

// MARK: - Chat Message Error

/// A structured error on a chat message (e.g. tool execution failure, model error).
...
// MARK: - Chat Completed Request

/// Request body for POST /api/chat/completed — notifies the server that streaming finished.
/// This triggers server-side post-completion filters, follow-up generation, and other hooks.
struct ChatCompletedRequest: Codable {
    let model: String
    let messages: [[String: String]]
    let chat_id: String
    let session_id: String
    let id: String              // assistant message ID
}
...
/// Empty body for POST requests that don't need a payload (share, clone, pin, archive).
struct EmptyBody: Codable {}

/// Request body for POST /api/v1/chats/{id}/folder
struct MoveChatToFolderRequest: Codable {
    let folder_id: String?
}
...
struct ChatBlob: Codable {
    ...
    let timestamp: Double?
    /// Model IDs used in this conversation
    let models: [String]?
    /// System prompt for the conversation
    let system: String?
    /// Model parameters (empty dict by default)
    let params: [String: String]?
    /// Tags for the conversation
    let tags: [String]?

    init(title: String, history: ChatBlobHistory, messages: [ChatBlobMessage],
         timestamp: Double? = nil, models: [String]? = nil, system: String? = nil,
         params: [String: String]? = nil, tags: [String]? = nil) {
        self.title = title
...
struct ChatBlobMessage: Codable {
    ...
    let codeExecutions: [ChatCodeExecution]?
    /// Follow-up suggestions
    let followUps: [String]?
    /// Token usage statistics
    let usage: TokenUsage?
    /// Structured error
    let messageError: ChatMessageError?
    /// Whether streaming is done (for assistant messages)
    let done: Bool?
    /// Model index (always 0 for single-model chats)
    let modelIdx: Int?

    enum CodingKeys: String, CodingKey {
        case id, role, content, model, parentId, childrenIds, timestamp
        case images, files, toolCalls, toolCallId, statusHistory
        case sources, followUps, usage, done, modelIdx
        case codeExecutions = "code_executions"
        case messageError = "error"
    }
}
