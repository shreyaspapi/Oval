<script lang="ts">
    ...
    let { state = $bindable(), id } = $props();
</script>

<Switch.Root
    bind:checked={state}
    {id}
    class="flex h-5 min-h-5 w-9 shrink-0 cursor-pointer items-center rounded-full px-[3px] mx-[1px] transition outline outline-1 outline-gray-100 dark:outline-gray-800 {state
        ? ' bg-emerald-600'
